...
var require_content_api = __commonJS({
  "node_modules/@tryghost/content-api/cjs/content-api.js"(exports, module2) {
    ...
    function GhostContentAPI2({ url, host, ghostPath = "ghost", version, key }) {
      if (host) {
      ...
      if (this instanceof GhostContentAPI2) {
        return GhostContentAPI2({ url, version, key });
      }
      if (!version) {
      ...
      const api = ["posts", "authors", "tags", "pages", "settings"].reduce((apiObject, resourceType) => {
        function browse(options2 = {}, memberToken) {
          return makeRequest(resourceType, options2, null, memberToken);
        }
        function read(data, options2 = {}, memberToken) {
          if (!data || !data.id && !data.slug) {
            return Promise.reject(new Error(`${name} read requires an id or slug.`));
          }
          const params = Object.assign({}, data, options2);
    ...
    module2.exports = GhostContentAPI2;
  }
});
...
init_shims();
var import_content_api = __toModule(require_content_api());
...
var Post = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  if ($$props.post === void 0 && $$bindings.post && post !== void 0)
    $$bindings.post(post);
  return `<main class="${"card text-black border-1 border-black rounded-none h-[100%]"}"><a href="${"/Blog/" + escape(post.slug)}"><div class="${"my-2 mx-2"}"><h1 class="${"font-bold"}">${escape(post.title.substring(0, 15))}...</h1>
            <p class="${"text-sm mt-auto"}">${escape(post.excerpt.substring(0, 50))}...</p></div></a></main>`;
});
async function load$1() {
  ...
  try {
    const jsonPosts = await api.posts.browse({ limit: 100, include: "tags, authors" });
...
var css = {
  code: "div.svelte-eipvvy img{width:max-content}",
  map: null
};
...
var U5Bslugu5D = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  if ($$props.post === void 0 && $$bindings.post && post !== void 0)
    $$bindings.post(post);
  $$result.css.add(css);
  return `<h1 class="${"text-center text-5xl bg-[#FFD8A7] text-[#0563C1] mb-8 py-20 font-bold"}">${escape(post.title)}</h1>
<div class="${"mx-7 max-w-[905] sm:text-md md:text-lg sm:mx-10 text-justify w-100% svelte-eipvvy"}"><!-- HTML_TAG_START -->${post.html}<!-- HTML_TAG_END -->
</div>`;
});
